    ...
    public class Email
    {
        ...
        public bool Send()
        {
            ...
            try
            {
                ...
                if (ShikshaConstants.UseSmtpCredentials.ToLower().Equals("true"))   // if false then use basic credentials 
                {
                    smtpClient.Host = ShikshaConstants.SmtpHost;
                    smtpClient.Port = Convert.ToInt32(ShikshaConstants.SmtpPort);
                    smtpClient.UseDefaultCredentials = ShikshaConstants.UseDefaultCredentials.ToLower().Equals("false") ? false : true;
                    smtpClient.Credentials = new NetworkCredential(ShikshaConstants.SmtpUsername, ShikshaConstants.SmtpPassword);
                }
                else
                {
                    smtpClient.Host = ShikshaConstants.DefaultHost;
                }

                smtpClient.EnableSsl = ShikshaConstants.EnableSsl.ToLower().Equals("true") ? true : false;
