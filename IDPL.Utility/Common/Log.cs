...
namespace Core.Utility.Common
{
    public static class Log
    {
        ...
        static Log()
        {
            ...
            Errorpath = ShikshaConstants.PhyPath + @"\Error\";
        ...
        public static void logMail(string logFile)
        {
            ...
            try
            {
                ...
                string FileUrl = ShikshaConstants.ShikshaDomain + @"/Bug/DownLoadFile?Id=" + HttpUtility.UrlEncode(EncryptDecrypt.EncryptNew(logFile));
        ...
        public static void WriteRazorPayLogWithoutMail(string fromModule, string fromMethod, string source, string logInfo)
        {
            try
            {
                Errorpath = ShikshaConstants.PhyPath + @"\Error\";
                ...
                string sDate = DateTime.Now.Date.ToString("yyyyMMMdd");
                // string Errorpath = System.AppDomain.CurrentDomain.BaseDirectory + @"\Error\";
                FileStream logFile = new FileStream(Errorpath + "[" + sDate + "]"+"_RazorPay.Infolog", FileMode.Append);
                StreamWriter streamWriter = new StreamWriter(logFile);
                ...
                streamWriter.WriteLine("{0} {1}", DateTime.Now.ToLongTimeString(), "");
                streamWriter.WriteLine("Module Name : " + fromModule + " (" + fromMethod + ")\n");
                streamWriter.WriteLine("Error Source : " + source + "\n ");
                streamWriter.WriteLine("Error Message : " + logInfo + "\n ");
                streamWriter.WriteLine(" \n ");
                streamWriter.WriteLine("======================================== \n ");
                streamWriter.Flush();
                streamWriter.Close();
